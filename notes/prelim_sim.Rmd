---
title: "FeBRILe3 - Preliminary Simulations"
...
Would this really be called a "non-inferiority" or just safety trial?

We should really include some measure of variability in the historical estimate. What data is this estimate based on? How far back does it go (based on last 12 months, last 10 years, may change over time)? How was it calculated? Do we have access to this data? Estimate of precision?
...
Assume that the historical baseline rate is $\theta_0$, and we will allow the policy change to produce the same rate within some tolerance $\delta$. The new rate under the policy change is $\theta$. 
...
For now we will assume $\theta_0$ is known exactly, however, it may be more realstic to assume some distribution $\theta_0\sim\text{Beta}(\alpha_0,\beta_0)$ such that $\alpha_0/(\alpha_0+\beta_0)$ is equal to the historical estimate. The variance could be based on how the estimate of the historical rate was obtained, or on our confidence in the estimate. Perhaps different rates should also be assumed for each site, but for now this is not considered.
...
x_k &= \text{the number of re-admissions within 7-days of discharge since stage }k-1 \\
...
$$
which has stopping time (sample size) $N_k = \min\left\{k\in\{1,...,K\}|\tau_k(y_k)=1,\sum_{j=1}^{k-1}\tau_j(y_{j}) = 0\right\}$.
...
example_trial <- sim_trial(nint = seq(50, 500, 50))
...
bind_rows(sc_1, sc_2, sc_3, sc_4, sc_5, sc_6, sc_7, sc_8, sc_9) %>%
  kable(escape = F, booktabs = T, align = "r",
        caption = "Scenario simulation results.", digits = c(0, rep(3, 3), 0, 0, rep(2, 2)),
