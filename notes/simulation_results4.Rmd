...
```{r priors, fig.height = 2.5, fig.width = 4, fig.cap="Priors considered."}
prior_uniform <- c(1, 1)
prior_sceptic <- unlist(betaExpert(0.11, lower = 0.08, p =0.8, method = "mean"))[1:2]
prior_enthuse <- unlist(betaExpert(0.05, upper = 0.08, p = 0.9, method = "mean"))[1:2]

ggplot(data.frame(x = c(0, 0.25)), aes(x)) +
  stat_function(aes(linetype = 'Uniform'), fun = dbeta, args = list(shape1 = 1, shape2 = 1)) +
  stat_function(aes(linetype = 'Sceptical'), fun = dbeta, 
                args = list(shape1 = prior_sceptic[1], shape2 = prior_sceptic[2])) +
  stat_function(aes(linetype = 'Enthusiastic'), fun = dbeta, 
                args = list(shape1 = prior_enthuse[1], shape2 = prior_enthuse[2])) +
  scale_linetype("") +
...
sim_dt[, ptail_sceptic := pbeta(0.08, a - 1 + prior_sceptic[1], b - 1 + prior_sceptic[2])]
sim_dt[, ptail_enthuse := pbeta(0.08, a - 1 + prior_enthuse[1], b - 1 + prior_enthuse[2])]
...
ggplot(data.frame(x = c(0, 0.2)), aes(x)) + 
  stat_function(fun = dbeta, args = list(shape1 = sim_res[1, a], shape2 = sim_res[1, b])) +
  stat_function(fun = dbeta, args = list(shape1 = sim_res[1, a] - 1 + prior_sceptic[1], 
                                         shape2 = sim_res[1, b] - 1 + prior_sceptic[2]),
                colour = "red") +
  stat_function(fun = dbeta, args = list(shape1 = sim_res[1, a] - 1 + prior_enthuse[1], 
                                         shape2 = sim_res[1, b] - 1 + prior_enthuse[2]),
                colour = "blue")
```
...
Pk_sceptic <- sprintf("%.2f", sim_res[sim_id == id_safe, ptail_sceptic])
Pk_enthuse <- sprintf("%.2f", sim_res[sim_id == id_safe, ptail_enthuse])
...
plotdat <- sim_dt[sim_id == id_safe, lapply(seq(0, 0.15, 0.001), 
  function(x) dbeta(x, a - 1 + prior_sceptic[1], b - 1 + prior_sceptic[2])), 
                  by = .(n, a = a - 1 + prior_sceptic[1], b = b - 1 + prior_sceptic[2])]
plotdat <- melt(plotdat, id.vars = c("n", "a", "b"), variable.name = "theta", value.name = "p")
plotdat[, theta := rep(seq(0, 0.15, 0.001), each = 9)]
plotdat[, q := qbeta(0.05, a, b)]
plotdat[, cp := ifelse(theta < 0.08, 1, 0)]
...
Pk_sceptic <- sprintf("%.2f", sim_res[sim_id == id_unsafe, ptail_sceptic])
Pk_enthuse <- sprintf("%.2f", sim_res[sim_id == id_unsafe, ptail_enthuse])
...
plotdat <- sim_dt[sim_id == id_unsafe, lapply(seq(0, 0.15, 0.001), 
  function(x) dbeta(x, a - 1 + prior_sceptic[1], b - 1 + prior_sceptic[2])), 
  by = .(n, a = a - 1 + prior_sceptic[1], b = b - 1 + prior_sceptic[2])]
plotdat <- melt(plotdat, id.vars = c("n", "a", "b"), variable.name = "theta", value.name = "p")
plotdat[, theta := rep(seq(0, 0.15, 0.001), each = 9)]
...
dec_res_df %>% 
  ...
  summarise(inf = mean(decision == "inferior"), 
            ...
            std_est = sd(est)) %>%
  arrange(ptru) %>%
  kable(format = "html", escape = F, digits = c(3, 3, 3, 1,1, 2, 2, 2, 0, 0, 2, 2), 
...
dec_res_df %>% 
  ...
  summarise(inf = mean(decision == "inferior"), 
            ...
            std_est = sd(est)) %>%
  arrange(ptru) %>%
  kable(format = "html", escape = F, digits = c(3, 3, 3, 2, 2, 2, 0, 0, 2, 2), 
...
```{r}
dec_res_df %>% 
  filter(a0 == 1, b0 == 1, k_inf == 0.05, k_non == 0.95) %>% 
  group_by(ptru, k_inf, k_non) %>%
  summarise(inf = mean(decision == "inferior"), 
            non = mean(decision == "non-inferior"),
            stop_early = mean(action == "stop"),
            ...
            std_est = sd(est)) %>%
  arrange(ptru) %>%
    kable(format = "html", escape = F, digits = c(2, 2, 2, 2, 2, 2, 0, 0, 2, 2), 
        linesep = c(rep('', 5)),
...
dec_res2 %>% 
  ...
  summarise(inf = mean(decision == "inferior"), 
            ...
            std_est = sd(est)) %>%
  arrange(ptru) %>%
  kable(format = "html", escape = F, digits = c(2, 2, 2, 2, 2, 0, 0, 2, 2), 
        linesep = c(rep('', 5)),
        caption = "Decision probabilities, stopping probability, and expected sample size for $\\underline c_k = 0.01 + 0.01125(k-1)$.",
